...
app.config['SQLALCHEMY_DATABASE_URI'] = \
    '{SGBD}://{usuario}:{clave}@{servidor}/{database}'.format(
        ...
        usuario = 'DiegoGMZ',
        clave = 'mysqlroot',
        servidor = 'DiegoGMZ.mysql.pythonanywhere-services.com',
        database = 'DiegoGMZ$pedidoEntrega'
    )
...
class Usuarios(db.Model):
    ...
    usuario = db.Column(db.String(20), primary_key=True) # Ojo: La columna se llama 'usuario'
...
# --- DATOS SIMULADOS (Mantenidos igual) ---
CLIENTES_DB = {
    "CLI001": {
        "codigo": "CLI001",
        "cliente": "Juan Pérez",
...
def login():
    if request.method == 'POST':
        ...
        # --- CORRECCIÓN AQUI ---
        # Usamos filter_by buscando en la columna 'usuario' el valor de la variable 'username'
        ...
        # Si el usuario existe, verificamos la clave
        if registro_usuario and registro_usuario.clave == password:
            session['username'] = registro_usuario.usuario
            session['role'] = registro_usuario.role
            flash(f'Bienvenido, {registro_usuario.usuario}!', 'success')
...
def ver_mapa():
    ...
    path='/home/DiegoGMZ/mysite/static/mapa_cbb.html'
    m.save(path)
